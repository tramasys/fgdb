...
pub(super) fn assign_string(
    ...
    if let Some(varobj) = variable.varobj.as_deref() {
        ...
        assign_resolved_string(ui, client, variable, expression, bytes, kind);
    }
}
...
fn assign_resolved_string(
    ...
    match kind {
        crate::ui::StringAssignmentKind::Buffer => {
            resolve_string_address(ui, client, variable, expression, bytes);
        }
        crate::ui::StringAssignmentKind::CppString => {
            assign_cpp_string(ui, client, variable, expression, bytes);
        }
        crate::ui::StringAssignmentKind::RustString => {
            assign_rust_string(ui, client, variable, expression, bytes);
        }
    }
}
...
fn resolve_rust_string_buffer(ui: Weak<Ui>, client: Rc<MiClient>, name: String, bytes: Vec<u8>) {
    let python = format!(
    ...
    if let Err(error) = client.request(&command, move |_, record| {
        ...
        if let Err(error) = client_for_response.request(
            "-data-evaluate-expression $fgdb_rs_data",
            move |_, record| {

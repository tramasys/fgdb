...
pub(super) fn request_misc_refresh(ui: Weak<Ui>, client: Rc<MiClient>) {
    ...
    if let Err(error) = client.request("-list-thread-groups", move |_, record| {
        ...
        read_process_startup(ui_for_response, generation, pid, debugger_pid);
    }) {
        show_misc_error(&ui, generation, &error.to_string());
    }
}

fn read_process_startup(ui: Weak<Ui>, generation: u64, pid: u32, debugger_pid: u32) {
    const READ_TIMEOUT: Duration = Duration::from_secs(5);
    if MISC_READER_ACTIVE.swap(true, Ordering::AcqRel) {
        show_misc_error(
            &ui,
            generation,
            "A previous process-vector reader is still finishing",
        );
        return;
    }
    let (sender, receiver) = mpsc::channel();
    let worker = std::thread::Builder::new()
        .name(String::from("fgdb-process-vectors"))
        .spawn(move || {
            let _guard = MiscWorkerGuard;
            let _ = sender.send(crate::kernel::read_process_startup(pid, debugger_pid));
        });
    if let Err(error) = worker {
        ...
        show_misc_error(
            &ui,
            generation,
            &format!("Cannot start the process-vector reader: {error}"),
        );
        return;
    }
        ...
        match receiver.try_recv() {
            ...
            Err(TryRecvError::Empty) if ui.strong_count() > 0 => {
                show_misc_error(
                    &ui,
                    generation,
                    "Reading the process startup vectors exceeded five seconds",
                );
                gtk::glib::ControlFlow::Break
            }
            Err(TryRecvError::Disconnected) => {
                show_misc_error(
                    &ui,
                    generation,
                    "The process-vector reader stopped before returning data",
                );
                gtk::glib::ControlFlow::Break
            }
